class EventRegistrationsController < ApplicationController
  ...
  def validate_registration
    can_attend = true
    registered = EventRegistration.find_by_application_id_and_event_id(params[:appln_id],params[:event_id])
    logger.info("#{registered}")
    if(registered.confirmation == params[:confirmation])
      if(registered.attended > 0) 
        registered.attended = registered.attended - 1
        registered.save
      else
        can_attend = false
      end
    ...
    respond_to do |format|
      format.json { render :json=>can_attend }
    end
  end
